...
module.exports.sendLink = async function(req,res){
   ...
   try{
    ...
    let fUser = await User.find({email: req.body.email});
    console.log(fUser);
    ...
    console.log(resPass);
    tokenObj = await User.populate(resPass,  { path:"user", model:"User" });
...
module.exports.finalReset = async function(req, res){
 ...
 try {
    ...
    let obj = await resetPass.find({accessToken: req.params.token});
    obj = await resetPass.populate(obj[0], {path: 'user', model: 'User'});
    console.log(obj);
    if(obj){

        if(obj.is_Valid ){
            return res.render('finalPassReset', {token: obj.accessToken, title: 'Reset Password'});
        }
        else{
...
try {
    ...
    if(req.body.pass1 === req.body.pass2){
        ...
        console.log(req.params.token);
        let obj = await resetPass.findOne({accessToken: req.params.token});
        console.log(obj);
        let fullObj = await resetPass.populate(obj, {  path:"user", model:"User" });
        console.log(fullObj);
        await resetPass.updateOne({accessToken: req.params.token}, {is_Valid: false});
        console.log(fullObj.user._id);
        await User.findByIdAndUpdate(  fullObj.user._id , {password: req.body.pass2});
